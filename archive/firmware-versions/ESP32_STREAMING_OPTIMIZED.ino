/*
 * ESP32 VOICE ASSISTANT - STREAMING MODE (Optimized)
 * ✅ 16kHz sample rate for clearer Arabic TTS
 * ✅ Periodic test beep (pauses during TTS)
 * ✅ Wake word confirmation beep (plays when server detects wake word)
 * ✅ STREAMING PLAYBACK - Handles unlimited response length!
 * ✅ Memory efficient - Only uses ~4KB
 */
...
#define MIC_BATCH_SIZE        3      // ✅ Optimized for mobile hotspot (middle ground)
#define MIC_SEND_QUEUE_SIZE   64     // ✅ Larger queue to handle jitter

// Speaker
#define SPK_WRITE_CHUNK_SIZE  2048
#define SPK_PLAY_QUEUE_SIZE   16     // ✅ Larger queue for mobile hotspot

// WebSocket send queue
#define WS_SEND_QUEUE_SIZE    32     // ✅ Larger queue
...
// Debug
#define DEBUG_WEBSOCKET  false  // No PONG spam
...
void handleTtsAudioChunkParsed(int chunkNumber, int totalChunks,
  ...
  if (chunkNumber == 0) {
    ...
    Serial.println();
    Serial.println("╔════════════════════════════════════════════════════════════╗");
    Serial.println("║         🎵 STREAMING AUDIO (16kHz, Unlimited Size)       ║");
    Serial.println("╚════════════════════════════════════════════════════════════╝");
  ...
  while (true) {
    if (xQueueReceive(spkPlayQueue, &c, portMAX_DELAY) == pdTRUE) {
      ...
      if (c.chunkNumber + 1 >= c.totalChunks) {
        ...
        // ✅ RESET BEEP TIMER after TTS finishes (so beep resumes after 2 seconds)
        g_lastBeepTime = millis();
      }
...
void onWebSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_CONNECTED:
      ...
      Serial.println("[MODE] 🎯 STREAMING (Handles unlimited response size!)");
...
void setup() {
  ...
  Serial.println("\n\n╔══════════════════════════════════════════════════════════╗");
  Serial.println("║      ESP32 VOICE ASSISTANT - STREAMING MODE            ║");
  Serial.println("║         🎯 Unlimited Response Size (Memory Efficient)  ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
  ...
  Serial.println("\n╔══════════════════════════════════════════════════════════╗");
  Serial.println("║         ✅ STREAMING MODE READY                          ║");
  Serial.println("║         🎯 Handles Any Response Size!                   ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
  
  Serial.printf("[INFO] Memory usage: ~4KB (vs 80-164KB for buffering)\n");
