...
void speakerPlaybackTaskFunction(void* parameter) {
    ...
    while (true) {
        if (speaker.isPlaying && speaker.audioBuffer != nullptr && speaker.bufferUsed > 0) {
            ...
            // ✅ DIAGNOSTIC: Check audio samples BEFORE gain
            int16_t* audioSamples = (int16_t*)speaker.audioBuffer;
            size_t sampleCount = speaker.bufferUsed / sizeof(int16_t);
            
            Serial.printf("[SPK-TASK] Sample count: %d\n", sampleCount);
            
            // Check first 10 samples
            Serial.print("[SPK-TASK] First 10 samples (BEFORE gain): ");
            ...
            Serial.printf("[SPK-TASK] Sample range: %d to %d\n", minSample, maxSample);
            
            // ✅ Check if all samples are zero (SILENT AUDIO!)
            if (nonZeroSamples == 0) {
                Serial.println("[SPK-TASK] ⚠️⚠️⚠️ WARNING: ALL SAMPLES ARE ZERO! ⚠️⚠️⚠️");
                Serial.println("[SPK-TASK] PC is sending SILENT AUDIO!");
                Serial.println("[SPK-TASK] This is why you hear nothing!");
            }
            
            // Apply volume gain to audio samples
            Serial.printf("[SPK-TASK] Applying volume gain: %.1fx\n", SPK_VOLUME_GAIN);
            ...
            // Check first 10 samples AFTER gain
            Serial.print("[SPK-TASK] First 10 samples (AFTER gain): ");
            for (int i = 0; i < 10 && i < sampleCount; i++) {
                Serial.printf("%d ", audioSamples[i]);
            }
            Serial.println();
            ...
            Serial.printf("[SPK-TASK] Expected duration: %lu ms (%d samples at %d Hz)\n",
                         expectedDurationMs, sampleCount, AUDIO_SAMPLE_RATE);
