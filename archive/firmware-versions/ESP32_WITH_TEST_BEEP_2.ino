...
#define BEEP_VOLUME           0.15f  // 15% volume (quiet beep)
...
// Generate a simple sine wave beep tone
void playTestBeep() {
  if (!BEEP_ENABLED) return;
  
  // Calculate number of samples for the beep duration
  size_t numSamples = (AUDIO_SAMPLE_RATE * BEEP_DURATION_MS) / 1000;
  ...
  // Generate sine wave at BEEP_FREQUENCY Hz
  float phase = 0.0f;
  float phaseIncrement = (TWO_PI * BEEP_FREQUENCY) / AUDIO_SAMPLE_RATE;
  ...
  for (size_t i = 0; i < numSamples; i++) {
    ...
    float sample = sinf(phase) * envelope * BEEP_VOLUME * 32767.0f;
...
void onWebSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_CONNECTED:
      ...
      if (BEEP_ENABLED) {
        Serial.printf("[BEEP] 🔔 Test beep running (every %dms, %dHz, %dms duration)\n", 
                      BEEP_INTERVAL_MS, BEEP_FREQUENCY, BEEP_DURATION_MS);
        Serial.println("[BEEP] ℹ️  Beep pauses during TTS playback");
      }
      break;
...
void setup() {
  ...
  Serial.println("║           🔔 WITH SPEAKER TEST BEEP 🔔                 ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
  ...
  if (BEEP_ENABLED) {
    Serial.println("[BEEP] 🔔 Speaker test beep will start in 2 seconds");
