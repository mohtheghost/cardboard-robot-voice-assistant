/*
 * ESP32 VOICE ASSISTANT - JITTER BUFFERING (Mobile Hotspot Optimized!)
 ...
 * ✅ JITTER BUFFER - Pre-buffers 10 chunks before playing!
 * ✅ Absorbs network jitter on mobile hotspot
 * ✅ Memory efficient (~20KB buffer)
 */
...
#define JITTER_BUFFER_SIZE    10     // Pre-buffer 10 chunks before playing
#define JITTER_BEEP_ENABLED   true   // Beep when buffering complete
...
#define SPK_PLAY_QUEUE_SIZE   20     // Bigger queue for jitter buffer
...
// ✅ JITTER BUFFER READY BEEP (short, medium pitch)
void playJitterBufferReadyBeep() {
  if (JITTER_BEEP_ENABLED) {
    generateBeep(1000, 100, 0.08f);  // 1000Hz, 100ms, 8% volume
  }
}
...
void handleTtsAudioChunkParsed(int chunkNumber, int totalChunks,
  ...
  if (chunkNumber == 0) {
    ...
    Serial.println();
    Serial.println("╔════════════════════════════════════════════════════════════╗");
    Serial.println("║         🎵 JITTER BUFFERING (Mobile Hotspot Optimized)   ║");
    Serial.println("╚════════════════════════════════════════════════════════════╝");
    Serial.printf("[JITTER] New stream id=%lu total=%d chunks\n", (unsigned long)g_ttsStreamId, totalChunks);
    Serial.printf("[JITTER] 📥 Buffering first %d chunks before playback...\n", JITTER_BUFFER_SIZE);
  }
  ...
  if (speaker.isBuffering) {
    ...
    if ((chunkNumber + 1) % 3 == 0 || (chunkNumber + 1) == JITTER_BUFFER_SIZE) {
      Serial.printf("[JITTER] 📥 Buffered %d/%d chunks...\n", chunkNumber + 1, JITTER_BUFFER_SIZE);
    }
    
    // ✅ JITTER BUFFER FULL - START PLAYBACK!
    if ((chunkNumber + 1) >= JITTER_BUFFER_SIZE) {
      speaker.isBuffering = false;
      Serial.println();
      Serial.println("╔════════════════════════════════════════════════════════════╗");
      Serial.println("║      ✅ JITTER BUFFER READY - STARTING PLAYBACK!          ║");
      Serial.println("╚════════════════════════════════════════════════════════════╝");
      Serial.printf("[JITTER] 🔊 Playing with %d-chunk safety buffer\n", JITTER_BUFFER_SIZE);
      
      // Play "ready" beep
      playJitterBufferReadyBeep();
    }
  } else {
    // Show receiving progress after buffering started
    if ((chunkNumber + 1) % 10 == 0 || (chunkNumber + 1) == totalChunks) {
      int progress = ((chunkNumber + 1) * 100) / totalChunks;
      Serial.printf("[SPK] Receiving: %d%% (%d/%d)\n",
...
void speakerPlaybackTaskFunction(void* parameter) {
  ...
  while (true) {
    if (xQueueReceive(spkPlayQueue, &c, portMAX_DELAY) == pdTRUE) {
      ...
      if ((c.chunkNumber + 1) % 20 == 0) {
        int playProgress = ((c.chunkNumber + 1) * 100) / c.totalChunks;
        Serial.printf("[SPK] Playing: %d%% (%d/%d)\n",
                      playProgress, c.chunkNumber + 1, c.totalChunks);
      }

      lastPlayedChunk = c.chunkNumber;

      if (c.chunkNumber + 1 >= c.totalChunks) {
        i2s_zero_dma_buffer(I2S_NUM_1);
        Serial.println();
        Serial.println("╔════════════════════════════════════════════════════════════╗");
        Serial.println("║         ✅ PLAYBACK COMPLETE (No Cuts on Mobile!)         ║");
        Serial.println("╚════════════════════════════════════════════════════════════╝");
...
void onWebSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_CONNECTED:
      ...
      Serial.println("[MODE] 🎯 JITTER BUFFERING (Mobile Hotspot Optimized!)");
      Serial.printf("[JITTER] Pre-buffer size: %d chunks (~%dKB)\n", 
                    JITTER_BUFFER_SIZE, (JITTER_BUFFER_SIZE * 2));
...
void setup() {
  ...
  Serial.println("\n\n╔══════════════════════════════════════════════════════════╗");
  Serial.println("║      ESP32 VOICE ASSISTANT - JITTER BUFFERING          ║");
  Serial.println("║         🎯 Mobile Hotspot Optimized (No Cuts!)         ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
  ...
  Serial.println("\n╔══════════════════════════════════════════════════════════╗");
  Serial.println("║         ✅ JITTER BUFFERING MODE READY                   ║");
  Serial.println("║         🎯 Pre-buffers 10 chunks before playing!        ║");
  Serial.println("║         📡 Perfect for Mobile Hotspot!                  ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
  
  Serial.printf("[INFO] Jitter buffer: %d chunks (~%dKB)\n", JITTER_BUFFER_SIZE, JITTER_BUFFER_SIZE * 2);
  Serial.printf("[INFO] Memory usage: ~%dKB for buffer\n", JITTER_BUFFER_SIZE * 2);
  ...
  if (BEEP_ENABLED) {
    ...
    Serial.println("[BEEP] 🎯 Buffering complete beep enabled\n");
  }
...
void loop() {
  ...
  if (BEEP_ENABLED && !speaker.isPlaying && !speaker.isBuffering) {
