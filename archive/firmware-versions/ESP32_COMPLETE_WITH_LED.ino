 ...
 * ║  ✅ LED Status Indicator - Blue LED shows connection status             ║
 ...
 * ║  • Blue LED            → GPIO 2 (built-in LED)                          ║
 * ║                                                                          ║
 * ║  LED Status:                                                             ║
 * ║  • LED OFF  = WebSocket Connected ✅                                    ║
 * ║  • LED ON   = WebSocket Disconnected ❌                                 ║
 * ║  • LED BLINK = Error or Starting Up                                     ║
 * ║                                                                          ║
 * ║  Libraries Required:                                                     ║
...
// LED Configuration
// ────────────────────────────────────────────────────────────────────────────
#define LED_PIN 2  // Built-in blue LED on most ESP32 boards (GPIO 2)
// If your board uses a different pin for LED, change this number
// Common alternatives: GPIO 5, GPIO 16, GPIO 17
...
#define WS_HEARTBEAT_INTERVAL_MS 5000   // ✅ More frequent pings
...
#define DEBUG_WEBSOCKET       false  // ✅ Disable verbose WS logging
#define DEBUG_MICROPHONE      false  // ✅ Disable verbose mic logging
#define DEBUG_SPEAKER         false  // ✅ Disable verbose speaker logging (MUCH faster!)
#define DEBUG_MEMORY          false  // ✅ Disable stats (improves performance)
...
void speakerPlaybackTaskFunction(void* parameter) {
    ...
    while (true) {
        ...
        vTaskDelay(pdMS_TO_TICKS(5));  // ✅ Faster loop (was 10ms)
    }
}
...
void onWebSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
    switch (type) {
        case WStype_DISCONNECTED:
            ...
            // ✅ Turn LED ON when disconnected
            ledOn();
            Serial.println("[LED] 💡 Blue LED ON (disconnected)");
        ...
        case WStype_CONNECTED:
            ...
            ledOff();
            Serial.println("[LED] 💡 Blue LED OFF (connected)");
...
void printSystemStatistics() {
    ...
    Serial.printf("║  LED Status: %-45s ║\n", wsConnected ? "OFF (Connected)" : "ON (Disconnected)");
...
void setup() {
    ...
    Serial.printf("[LED] 💡 LED initialized on GPIO %d\n", LED_PIN);
    
    // ✅ Blink LED 3 times on startup (visual confirmation)
    ledBlink(3, 200);
    ...
    Serial.println("║                      WITH LED STATUS INDICATOR                           ║");
    Serial.println("║                                                                          ║");
    Serial.println("║  Dual I2S Voice Assistant with Full-Duplex Audio                         ║");
    Serial.println("║  Microphone (INMP441) + Speaker (MAX98357A)                             ║");
    Serial.println("║                                                                          ║");
    Serial.println("║  💡 LED Status:                                                          ║");
    Serial.println("║     • OFF = WebSocket Connected ✅                                       ║");
    Serial.println("║     • ON  = WebSocket Disconnected ❌                                    ║");
    Serial.println("║                                                                          ║");
    printDivider('═');
    Serial.println();
    ...
    if (!initializeSpeaker()) {
        ...
        ledBlink(10, 100);  // ✅ Rapid blink on fatal error
        while (1) delay(1000);
    }
    delay(100);
    ...
    while (WiFi.status() != WL_CONNECTED) {
        ...
        if (++ledToggleCount % 4 == 0) {
            digitalWrite(LED_PIN, !digitalRead(LED_PIN));
        }
    }
    
    ledOff();  // ✅ Turn LED off after WiFi connected
    ...
    // ✅ LED will be ON until WebSocket connects (starts disconnected)
    ledOn();
    Serial.println("[LED] 💡 LED ON (waiting for WebSocket connection)");
    ...
    Serial.println("║  💡 LED Indicator: ACTIVE                                 ║");
    Serial.println("║      • LED OFF = Connected ✅                             ║");
    Serial.println("║      • LED ON  = Disconnected ❌                          ║");
    Serial.println("║                                                            ║");
...
void loop() {
    // ✅ Service WebSocket connection FIRST AND OFTEN!
    webSocket.loop();
    
    // Check if WebSocket is connected
    if (!wsConnected) {
        delay(5);  // ✅ Shorter delay (was 10ms)
        return;
    }
    ...
    // ✅ No delay at end - loop as fast as possible!
    // This ensures webSocket.loop() is called frequently
}
