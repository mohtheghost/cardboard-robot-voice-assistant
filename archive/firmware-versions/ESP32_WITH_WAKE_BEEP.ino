...
#define BEEP_VOLUME           0.05f  // 5% volume (quiet beep)
...
#define WAKE_BEEP_VOLUME      0.10f  // Louder than test beep (10% vs 5%)
...
void onWebSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_CONNECTED:
      ...
      if (BEEP_ENABLED) {
        Serial.printf("[BEEP] 🔔 Test beep running (every %dms, %dHz, %dms, %.1f%% volume)\n", 
                      BEEP_INTERVAL_MS, BEEP_FREQUENCY, BEEP_DURATION_MS, BEEP_VOLUME * 100);
...
void setup() {
  ...
  Serial.println("║           🔔 WITH WAKE WORD CONFIRMATION BEEP 🔔       ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
