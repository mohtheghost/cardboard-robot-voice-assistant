...
void handleTtsAudioChunk(JsonDocument& doc) {
    ...
    if (decodedLength > 0) {
        ...
        unsigned long timeoutMs = expectedDurationMs + 1000;  // Add 1s safety margin
        
        if (DEBUG_SPEAKER && chunkNumber < 5) {
            Serial.printf("[SPK] Waiting for playback (duration: %d ms)\n", expectedDurationMs);
        }
        
        // ✅ Wait for chunk to FINISH, but call webSocket.loop() frequently!
        unsigned long waitStart = millis();
        while (speaker.isPlaying && (millis() - waitStart < timeoutMs)) {
            webSocket.loop();  // ✅ Keep connection alive!
            delay(5);          // ✅ Call loop every 5ms
        }
...
void speakerPlaybackTaskFunction(void* parameter) {
    ...
    while (true) {
        if (speaker.isPlaying && speaker.audioBuffer != nullptr && speaker.bufferUsed > 0) {
            ...
            Serial.printf("[SPK-TASK] Waiting for audio to finish...\n");
            
            // Wait for the audio to finish, but keep WebSocket alive
            unsigned long waitStart = millis();
            while ((millis() - waitStart) < expectedDurationMs) {
                webSocket.loop();  // Keep connection alive
                delay(5);
            }
            
            unsigned long actualWait = millis() - waitStart;
            Serial.printf("[SPK-TASK] ✅ Waited %lu ms, audio finished\n", actualWait);
