...
#define SPK_VOLUME_GAIN       5.0f  // ✅ Volume amplification - 5x LOUDER! (was 3.0f)
...
void handleTtsAudioChunk(JsonDocument& doc) {
    ...
    // ✅ FIX: If we get a non-zero chunk but no buffer, treat it as a new stream
    if (chunkNumber != 0 && speaker.audioBuffer == nullptr) {
        Serial.printf("[SPK] ⚠️  Received chunk %d but no buffer - treating as chunk 0\n", chunkNumber);
        chunkNumber = 0;  // Force reset
    }
    
    // ✅ FIX: If chunks seem out of order, reset
    if (speaker.audioBuffer != nullptr && chunkNumber != (speaker.currentChunk + 1) && chunkNumber != 0) {
        Serial.printf("[SPK] ⚠️  Chunk sequence error: got %d, expected %d - resetting\n", 
                     chunkNumber, speaker.currentChunk + 1);
        
        // Free old buffer and start fresh
    ...
    // First chunk - initialize playback
    if (chunkNumber == 0) {
        Serial.println("\n╔════════════════════════════════════════════════════════════╗");
        ...
        if (speaker.audioBuffer != nullptr) {
            Serial.println("[SPK] ⚠️  Freeing old buffer from previous stream");
            free(speaker.audioBuffer);
            speaker.audioBuffer = nullptr;
        }
        
        // Allocate buffer for ALL chunks (estimate ~2KB per chunk)
        size_t estimatedSize = totalChunks * 2048;
        if (estimatedSize > 150000) estimatedSize = 150000; // Cap at 150KB
        
        Serial.printf("[SPK] Allocating %s for %d chunks...\n", 
                     formatBytes(estimatedSize).c_str(), totalChunks);
        
        speaker.audioBuffer = (uint8_t*)malloc(estimatedSize);
        if (speaker.audioBuffer == nullptr) {
        ...
        speaker.bufferSize = estimatedSize;
        speaker.bufferUsed = 0;  // ✅ Start at 0, will accumulate
        speaker.currentChunk = -1;  // Will be set to 0 below
    ...
    if (speaker.audioBuffer == nullptr) {
        Serial.println("[SPK] ❌ Audio buffer STILL not initialized after reset attempt!");
        Serial.printf("[SPK] This shouldn't happen - memory issue?\n");
        Serial.printf("[SPK] Free heap: %s\n", formatBytes(ESP.getFreeHeap()).c_str());
        return;
    }
    ...
    size_t maxDecodedSize = (base64Length * 3) / 4 + 4;  // Estimate decoded size
    
    if (speaker.bufferUsed + maxDecodedSize > speaker.bufferSize) {
        Serial.printf("[SPK] ⚠️  Buffer full! Used: %s, Need: %s, Total: %s\n",
                     formatBytes(speaker.bufferUsed).c_str(),
                     formatBytes(maxDecodedSize).c_str(),
                     formatBytes(speaker.bufferSize).c_str());
        return;
    }
    
    // Decode base64 data and APPEND to buffer
    size_t decodedLength = 0;
    
    // ✅ Decode to buffer position (append, don't overwrite!)
    int result = mbedtls_base64_decode(
        speaker.audioBuffer + speaker.bufferUsed,  // ✅ Append at current position
        speaker.bufferSize - speaker.bufferUsed,   // ✅ Remaining space
        &decodedLength,
    ...
    if (result != 0) {
        ...
        Serial.printf("[SPK] Buffer space remaining: %s\n", 
                     formatBytes(speaker.bufferSize - speaker.bufferUsed).c_str());
        return;
    }
    ...
    if ((chunkNumber + 1) % 5 == 0 || (chunkNumber + 1) == totalChunks) {
        ...
        Serial.printf("[SPK] 📥 Progress: %d%% (%d/%d chunks, %s total)\n",
                     progress, chunkNumber + 1, totalChunks, formatBytes(speaker.bufferUsed).c_str());
    }
    
    // ✅ Last chunk - NOW play the complete audio
    if ((chunkNumber + 1) >= totalChunks) {
        Serial.println("\n╔════════════════════════════════════════════════════════════╗");
        Serial.println("║           ✅ ALL CHUNKS RECEIVED - PLAYING NOW             ║");
        Serial.println("╚════════════════════════════════════════════════════════════╝");
        
        Serial.printf("[SPK] Total audio size: %s\n", formatBytes(speaker.bufferUsed).c_str());
        float durationSec = speaker.bufferUsed / 2.0 / AUDIO_SAMPLE_RATE;
        Serial.printf("[SPK] Duration: %.1f seconds\n", durationSec);
        ...
        unsigned long expectedDuration = (unsigned long)(durationSec * 1000) + 2000; // Add 2s buffer
        
        while (speaker.isPlaying && (millis() - playbackStart < expectedDuration)) {
            vTaskDelay(pdMS_TO_TICKS(100));
        }
        
        if (speaker.isPlaying) {
            Serial.println("[SPK] ⚠️  Playback timeout - forcing stop");
            speaker.isPlaying = false;
        }
...
void speakerPlaybackTaskFunction(void* parameter) {
    ...
    while (true) {
        if (speaker.isPlaying && speaker.audioBuffer != nullptr && speaker.bufferUsed > 0) {
            if (DEBUG_SPEAKER) {
                Serial.printf("[SPK-TASK] 🔊 Playing %s (Gain: %.1fx)...\n", 
                             formatBytes(speaker.bufferUsed).c_str(), SPK_VOLUME_GAIN);
            }
