/*
 * ESP32 VOICE ASSISTANT - BUFFER-THEN-PLAY (Memory Optimized!)
 ...
 * ✅ REDUCED BUFFER SIZE (80KB) - Prevents allocation failures
 */
...
// Debug
#define DEBUG_WEBSOCKET  false  // Changed to false to reduce PONG spam
#define DEBUG_MICROPHONE false
#define DEBUG_SPEAKER    false

// ✅ TTS BUFFER SETTINGS (REDUCED from 150KB to 80KB)
#define MAX_TTS_BUFFER_SIZE   (80 * 1024)  // 80KB max (safe for ESP32)
...
bool initTtsBuffer(int totalChunks, uint32_t streamId) {
  ...
  // ✅ CONSERVATIVE ESTIMATE: Start with smaller buffer
  // Estimate ~3KB per chunk instead of 4KB
  size_t estimatedSize = totalChunks * 3072;
  ...
  Serial.printf("[MEM] Free heap before allocation: %s\n", formatBytes(freeHeap).c_str());
  
  if (estimatedSize > freeHeap - 40960) {  // Keep 40KB free
    estimatedSize = freeHeap - 40960;
    if (estimatedSize < 16384) {
      Serial.println("[TTS] ❌ Not enough memory!");
      return false;
    }
  }
  ...
  if (!g_ttsBuffer.buffer) {
    Serial.printf("[TTS] ❌ Failed to allocate %s buffer! (Free heap: %s)\n", 
                  formatBytes(estimatedSize).c_str(), formatBytes(freeHeap).c_str());
    return false;
  }
  ...
  Serial.printf("[MEM] Free heap after allocation: %s\n", formatBytes(ESP.getFreeHeap()).c_str());
...
bool addTtsChunk(const uint8_t* data, size_t length) {
  ...
  if (g_ttsBuffer.currentOffset + length > g_ttsBuffer.bufferSize) {
    ...
    // Check free heap before realloc
    size_t freeHeap = ESP.getFreeHeap();
    if (newSize - g_ttsBuffer.bufferSize > freeHeap - 40960) {
      Serial.printf("[TTS] ❌ Not enough memory to expand! (Free: %s, Need: %s)\n",
                    formatBytes(freeHeap).c_str(), 
                    formatBytes(newSize - g_ttsBuffer.bufferSize).c_str());
      return false;
    }
...
void playBufferedAudio() {
  ...
  Serial.printf("[MEM] Free heap after playback: %s\n", formatBytes(ESP.getFreeHeap()).c_str());
...
void onWebSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
    ...
    case WStype_CONNECTED:
      ...
      Serial.println("[MODE] 🎯 Buffer-then-Play (Memory Optimized 80KB)");
...
void setup() {
  ...
  Serial.println("║         🎯 MEMORY OPTIMIZED (80KB Max Buffer)          ║");
  Serial.println("╚══════════════════════════════════════════════════════════╝\n");
