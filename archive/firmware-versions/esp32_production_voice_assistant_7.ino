...
void handleTtsAudioChunk(JsonDocument& doc) {
    ...
    if ((chunkNumber + 1) % 10 == 0 || (chunkNumber + 1) == totalChunks) {
        ...
        Serial.printf("[SPK] 📥 Streaming: %d%% (%d/%d chunks)\n",
                     progress, chunkNumber + 1, totalChunks);
    }
    
    // ✅ PLAY THIS CHUNK IMMEDIATELY (streaming)
    if (decodedLength > 0) {
        speaker.isPlaying = true;
        
        // Wait for playback (but not too long)
        unsigned long waitStart = millis();
        while (speaker.isPlaying && (millis() - waitStart < 2000)) {
