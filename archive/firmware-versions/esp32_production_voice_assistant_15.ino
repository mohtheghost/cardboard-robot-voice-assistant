...
// ✅ NEW: Background noise configuration (helps detect wiring issues!)
#define ENABLE_BACKGROUND_NOISE  true   // Set to false to disable
#define NOISE_VOLUME             50     // Volume of background noise (0-500, lower = quieter)
...
void speakerPlaybackTaskFunction(void* parameter) {
    ...
    const size_t NOISE_BUFFER_SIZE = 512;  // Small buffer for noise
    int16_t* noiseBuffer = nullptr;
    
    if (ENABLE_BACKGROUND_NOISE) {
        noiseBuffer = (int16_t*)malloc(NOISE_BUFFER_SIZE * sizeof(int16_t));
        if (noiseBuffer != nullptr) {
            // Generate white noise (random values)
            for (int i = 0; i < NOISE_BUFFER_SIZE; i++) {
                noiseBuffer[i] = (random(-NOISE_VOLUME, NOISE_VOLUME));
            }
            Serial.println("[SPK-TASK] ✅ Background noise enabled (helps detect wiring issues)");
            Serial.printf("[SPK-TASK] 🔊 Noise volume: %d (adjust NOISE_VOLUME to change)\n", NOISE_VOLUME);
        } else {
            Serial.println("[SPK-TASK] ⚠️  Failed to allocate noise buffer");
        }
    }
    
    unsigned long lastNoiseTime = millis();
    
    while (true) {
        // ✅ PRIORITY 1: Play TTS audio if available
        if (speaker.isPlaying && speaker.audioBuffer != nullptr && speaker.bufferUsed > 0) {
            Serial.println("\n[SPK-TASK] ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
            Serial.printf("[SPK-TASK] 🔊 PLAYBACK STARTING at %lu ms\n", millis());
            Serial.printf("[SPK-TASK] 🔇 Background noise PAUSED for TTS\n");
            ...
            Serial.println("[SPK-TASK] 🔊 Background noise will resume after silence");
            ...
            // ✅ Wait 2 seconds before resuming noise (let TTS finish)
            lastNoiseTime = millis() + 2000;
        }
        // ✅ PRIORITY 2: Play background noise if enabled and no TTS playing
        else if (ENABLE_BACKGROUND_NOISE && noiseBuffer != nullptr) {
            // Only play noise every 100ms to avoid overwhelming the system
            if (millis() - lastNoiseTime >= 100) {
                size_t bytesWritten = 0;
                esp_err_t result = i2s_write(
                    I2S_NUM_1,
                    noiseBuffer,
                    NOISE_BUFFER_SIZE * sizeof(int16_t),
                    &bytesWritten,
                    pdMS_TO_TICKS(10)  // Short timeout
                );
                
                if (result == ESP_OK && bytesWritten > 0) {
                    // Noise playing successfully
