...
void i2s_install_microphone() {
  const i2s_config_t cfg = {
    ...
    .use_apll             = false,  // ✅ Changed from true - fixes MCLK error
...
void handleTtsChunk(JsonDocument& doc) {
  ...
  if (chunkNum == 0) {
    ...
    // ✅ Use largest contiguous free block, not total free heap
    size_t largestBlock = ESP.getMaxAllocHeap();
    Serial.printf("[TTS] 💾 Largest free block: %d KB\n", largestBlock / 1024);
    
    // ✅ Leave 30KB safety margin for decoding/processing
    if (largestBlock < (30 * 1024)) {
      Serial.println("[TTS] ❌ Not enough memory for audio buffer!");
      tts.receiving = false;
      return;
    }
    
    size_t bufferSize = largestBlock - (30 * 1024);  // Leave 30KB margin
    
    Serial.printf("[TTS] 📦 Allocating %d KB for audio\n", bufferSize / 1024);
    
    tts.bufferSize = bufferSize;
    tts.bufferUsed = 0;
    
    tts.buffer = (uint8_t*)malloc(tts.bufferSize);
    
    if (!tts.buffer) {
      Serial.printf("[TTS] ❌ Allocation failed!\n");
      Serial.printf("[TTS] 💾 Tried to allocate: %d KB\n", tts.bufferSize / 1024);
      tts.receiving = false;
      return;
    }

    Serial.printf("[TTS] ✅ Buffer allocated: %d KB\n", tts.bufferSize / 1024);
    Serial.printf("[TTS] 📊 Can store ~%.1f seconds of audio\n", 
                  tts.bufferSize / 2.0 / SAMPLE_RATE);
