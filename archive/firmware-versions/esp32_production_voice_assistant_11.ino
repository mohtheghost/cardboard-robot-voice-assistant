...
void handleTtsAudioChunk(JsonDocument& doc) {
    ...
    if (chunkNumber == 0 || speaker.currentChunk == -1 || speaker.audioBuffer == nullptr) {
        ...
        Serial.println("[SPK] ⏳ Buffering first 3 chunks before playback...");
    }
    ...
    // ✅ BUFFERING: Wait for first 3 chunks before starting playback
    static bool isBuffering = true;
    
    if (chunkNumber < 3 && isBuffering) {
        Serial.printf("[SPK] ⏳ Buffering... (%d/3 chunks ready)\n", chunkNumber + 1);
        if (chunkNumber == 2) {
            Serial.println("[SPK] ✅ Buffer ready! Starting playback now...");
            isBuffering = false;
        }
    }
    
    // ✅ PLAY THIS CHUNK (but not first 3 if still buffering)
    if (decodedLength > 0 && (!isBuffering || chunkNumber >= 2)) {
        speaker.isPlaying = true;
        
        // Calculate expected playback duration
        size_t expectedDurationMs = ((decodedLength / 2) * 1000) / AUDIO_SAMPLE_RATE;
        unsigned long timeoutMs = expectedDurationMs + 500;  // Add 500ms safety margin
        
        if (DEBUG_SPEAKER && chunkNumber < 5) {
            Serial.printf("[SPK] Waiting for playback (expected: %d ms, timeout: %lu ms)\n",
                         expectedDurationMs, timeoutMs);
        }
        
        // ✅ Wait just 30ms for I2S to start, then ready for next chunk
        unsigned long shortWait = 30;  // ✅ Minimal wait to prevent gaps!
        
        if (DEBUG_SPEAKER && chunkNumber < 5) {
            Serial.printf("[SPK] Quick wait %lu ms (I2S DMA handles buffering)\n", shortWait);
        }
        
        unsigned long waitStart = millis();
        while (speaker.isPlaying && (millis() - waitStart < shortWait)) {
        ...
        if (speaker.isPlaying) {
            // Timeout - force stop
            Serial.printf("[SPK] ⚠️  Playback timeout after %lu ms!\n", millis() - waitStart);
            speaker.isPlaying = false;
        } else if (DEBUG_SPEAKER && chunkNumber < 5) {
            Serial.printf("[SPK] ✅ Playback completed in %lu ms\n", millis() - waitStart);
        }
    }
    
    // Last chunk - cleanup
    if ((chunkNumber + 1) >= totalChunks) {
        isBuffering = true;  // Reset for next TTS
...
void speakerPlaybackTaskFunction(void* parameter) {
    ...
    while (true) {
        if (speaker.isPlaying && speaker.audioBuffer != nullptr && speaker.bufferUsed > 0) {
            ...
            size_t sampleCount = totalWritten / 2;  // 16-bit samples
            ...
            // ✅ SHORT wait to let I2S start consuming data
            // Don't wait full duration - let DMA buffer handle queueing!
            Serial.printf("[SPK-TASK] Waiting 30ms for I2S to start...\n");
            
            for (int i = 0; i < 6; i++) {  // 30ms total
                webSocket.loop();  // Keep connection alive
                delay(5);
            }
            
            Serial.printf("[SPK-TASK] ✅ Ready for next chunk at %lu ms\n", millis());
